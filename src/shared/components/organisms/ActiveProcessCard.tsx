...
export function ActiveProcessCard({
  ...
  const cfg = slugConfig[slugForDisplay] ?? {
    ...
    label: slugForDisplay.toUpperCase(),
    category: "",
  };
  ...
  const isRFE = stepData.uscis_official_result === "rfe" || stepData.uscis_rfe_result === "rfe";
  ...
  return (
    <Link to={`/dashboard/processes/${proc.service_slug}?id=${proc.id}`} className="w-full h-full">
      <motion.div
        ...
        className={cn(
          "relative w-full rounded-[32px] border-2 bg-card p-6 sm:p-10 shadow-xl shadow-primary/5 transition-all hover:shadow-2xl hover:shadow-primary/10 cursor-pointer",
          isApproved ? "border-emerald-500/30 shadow-emerald-500/5" :
          isDenied ? "border-red-500/30 shadow-red-500/5" :
          isRFE ? "border-amber-500 shadow-amber-500/5" : "border-primary"
        )}
      >
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6 sm:mb-10">
          ...
          <span className={cn(
            ...
            isApproved ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" :
            isDenied ? "text-red-500 bg-red-500/10 border-red-500/20" :
            isRFE ? "text-amber-600 bg-amber-500/10 border-amber-500/20" :
            isFinalized ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" :
        ...
        <div className="flex items-center gap-2 mb-8">
          {isApproved ? (
            ...
            <RiCheckboxCircleFill className="text-red-500 text-base rotate-180" />
             ...
             <RiFlashlightFill className="text-amber-500 text-base animate-pulse" />
          ) : (
            <RiFlashlightFill className="text-text-muted text-base rotate-12" />
          )}
          <span className={cn(
            "text-[12px] font-bold tracking-widest uppercase",
            isApproved ? "text-emerald-500" : isDenied ? "text-red-500" : isRFE ? "text-amber-600" : "text-text-muted"
          )}>
            {isApproved ? labels.status.uscisApproved :
             isDenied ? labels.status.deniedEncerrado :
             isRFE ? labels.status.awaitingRfe :
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between font-black">
            <span className="text-[10px] text-text-muted uppercase tracking-widest">{labels.progress}</span>
            <span className="text-xl text-text">{progress}%</span>
          </div>
          <div className="relative w-full h-3 bg-bg-subtle rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
          ...
          <span className={cn(
            ...
            isFinalized ? 'text-emerald-500' : 'text-primary'
          )}>{progress}%</span>
        </div>
      </motion.div>
    </Link>
