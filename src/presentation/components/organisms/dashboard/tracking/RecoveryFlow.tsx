...
export const RecoveryFlow: React.FC<RecoveryFlowProps> = (props) => {
  ...
  const getRecoveryType = () => {
    // Status takes top priority: _RFE in status always means rfe flow
    if (status?.includes("_RFE")) return "rfe";

    // Explicit metadata takes priority over all fallbacks
    const metadata = process?.service_metadata as any;
    if (metadata?.recovery_type) return metadata.recovery_type; // 'rfe' or 'motion'

    // Legacy: check recovery case
    if (recoveryCase?.recovery_type === "rfe") return "rfe";
    
    // Last fallback: process.data (may be stale — use cautiously)
  ...
  useEffect(() => {
    ...
    // When in "motion after RFE rejection" state:
    // The user must manually contract the Motion — do NOT auto-advance based on old RFE payments.
    // This prevents rfe_recovery payments from triggering COS_MOTION_IN_PROGRESS prematurely.
    const isAwaitingMotionContract = status.endsWith("_REJECTED") && getRecoveryType() === 'motion';

    // Check for payments when in these specific states
    const shouldCheck = 
      !isAwaitingMotionContract && (
        isRfeFlow ||
        ...
        try {
          ...
          if (!error && data && isSubscribed) {
             console.log(`[RecoveryFlow] Found ${data.length} recent paid orders for ${userEmail || process.userId}. Checking matches for process ${process.id}...`);
              ...
              // Debug: log all orders to see why none match
              data.forEach((order, i) => {
                ...
                console.log(`[RecoveryFlow] Order[${i}] serviceId=${orderServiceId}, action=${orderAction}, process.id=${process.id}, match=${orderServiceId === process.id}`);
              });
              ...
              if (paidOrder) {
                ...
                const isGenericRejected = status === "rejected";
                ...
                // Fix nextStatus for generic 'rejected' → treat as _REJECTED for status computation
                if (isGenericRejected && !nextStatus) {
                  // A payment was found but nextStatus was blocked by endsWith("_REJECTED") check
                  // Recalculate: for analyst payments on rejected, go to CASE_FORM
                  const prefix = isEos ? 'EOS' : 'COS';
                  if (['rfe_analyst', 'rfe_support', 'cos_analyst', 'eos_analyst'].includes(orderAction)) {
                    nextStatus = `${prefix}_CASE_FORM`;
                  } else {
                    nextStatus = isRfeFlow ? `${prefix}_RFE_IN_PROGRESS` : `${prefix}_MOTION_IN_PROGRESS`;
                  }
                }
  ...
  // 1. Final Stage: Case Completed (Success)
  if (status.includes("_COMPLETED") || recoveryCase?.status === 'completed') {
    return (
      <motion.div 
  ...
  // 2. Stage: Specialist is preparing the delivery
  if (status.includes("_IN_PROGRESS") || recoveryCase?.status === 'accepted') {
  ...
  // 3. Analysis Proposal Stage
  // IMPORTANT: Skip this block if the status is _REJECTED and recovery_type is 'motion'
  // In that case the old RFE recovery case (proposal_sent) would incorrectly intercept
  // and show the RFE delivery screen again instead of the Motion initial screen.
  const isMotionAfterRfeRejection = status.endsWith("_REJECTED") && getRecoveryType() === 'motion';
  if (!isMotionAfterRfeRejection && (status.includes("ANALISE_CONCLUIDA") || recoveryCase?.status === 'proposal_sent')) {
    const isRfe = isRfeType;
    const checkoutLabel = isRfe ? 'Apoio ao RFE' : 'Motion de Reconsideração';
    
    // Use slugs that are already in the server's known catalog
    let checkoutSlug = isRfe
      ? (isEos ? 'analise-especialista-eos' : 'analise-especialista-cos')
      : (isEos ? 'motion-reconsideracao-eos' : 'motion-reconsideracao-cos');
      ...
      >
              ...
              <div className="flex flex-col items-center justify-center rounded-3xl bg-white/10 p-6 backdrop-blur-xl border border-white/20 shadow-xl transition-all hover:bg-white/15 min-w-[200px]">
                ...
                <div className="flex items-baseline gap-1 mt-1 drop-shadow-md">
                  ...
                  <span className="text-5xl font-black text-white tracking-tight">
                    {recoveryCase.proposal_value_usd || "0"}
                  </span>
                </div>
        ...
        <div className="pt-4 pb-12 w-full max-w-xl mx-auto">
          <Button 
            ...
            onClick={() => navigate(`/checkout/${checkoutSlug}?serviceId=${process.id}&action=${action}&amount=${recoveryCase?.proposal_value_usd || "0"}`)}
          >
  ...
  // 6. Initial Stage: RFE Received, Rejected, or Generic Rejection → Motion screen
  if (status.endsWith("_REJECTED") || status === "rejected" || isRfeStatus) {
