...
export const HeroSection = ({ user, heroImage, avatars }: HeroSectionProps) => {
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-24 items-center">
        <div className="z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              {user ? (
                ...
                <Link to="/servicos" className="px-10 py-5 bg-primary text-white font-bold text-lg rounded-xl shadow-2xl shadow-primary/30 hover:scale-105 transition-transform flex items-center justify-center gap-2">
                  {t.hero.getStarted} <span className="material-symbols-outlined">arrow_forward</span>
                </Link>
              )}
          ...
          <div className="mt-16 flex items-center gap-4">
            ...
            <p className="text-sm font-bold text-slate-400 tracking-wide">{t.hero.approvedCount}</p>
          </div>
        </div>
        <motion.div 
