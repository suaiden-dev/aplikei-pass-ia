    ...
    <>
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-100">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 h-24 flex items-center">
          {/* Logo */}
          <div className="flex-shrink-0 mr-4 lg:mr-8 xl:mr-12">
          ...
          {/* Right Actions */}
          <div className="flex items-center gap-3 lg:gap-8 flex-shrink-0 ml-auto">
            ...
            {user ? (
              ...
              <>
                <Link to="/login" className="hidden min-[1100px]:block font-bold text-slate-500 hover:text-primary transition-colors whitespace-nowrap text-sm xl:text-base">
                  {t.nav.login[lang]}
                </Link>
                <Link to="/cadastro" className="hidden sm:block px-6 xl:px-9 py-3.5 bg-highlight text-white font-bold rounded-full hover:shadow-2xl hover:shadow-highlight/30 transition-all whitespace-nowrap text-sm xl:text-base border-b-4 border-black/10 active:border-b-0 active:translate-y-1 hover:-translate-y-0.5">
      ...
      {/* Mobile nav - Moved outside nav for true full-screen fixed positioning */}
      {open && (
        <div className="min-[1100px]:hidden fixed inset-0 bg-white z-[100] p-6 pb-12 flex flex-col gap-8 overflow-y-auto">
          {/* Mobile Menu Header */}
          <div className="flex items-center justify-between mb-2">
            <Link to="/" onClick={() => setOpen(false)} className="flex items-center gap-2.5 group">
              <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center font-black text-2xl text-white">A</div>
              <span className="text-xl font-extrabold tracking-tighter text-dark-grey uppercase">Aplikei</span>
            </Link>
            <button 
          ...
          <div className="grid gap-3">
            <span className="font-bold text-slate-400 text-[10px] uppercase tracking-widest pl-2 mb-1">Menu</span>
            {navLinks.map((link) => (
              <Link 
                key={link.to} 
                to={link.to} 
                onClick={() => setOpen(false)}
                className={`flex items-center justify-between font-bold p-5 rounded-2xl transition-all text-lg ${location.pathname === link.to ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'bg-slate-50 text-slate-700 active:bg-slate-100 border border-slate-100'}`}
              >
                {link.label}
