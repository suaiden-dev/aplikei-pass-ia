...
export const CompanionsStep = ({ register, watch, setValue, lang, t }: StepProps) => {
    const ds = t.ds160;
    ...
    return (
        <div className="space-y-6 fade-in">
            <h2 className="font-display text-lg font-semibold text-foreground">{ds.companions.title[lang]}</h2>
            ...
            <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground mt-1">{ds.companions.companionHelper[lang]}</p>
                ...
                {hasTravelCompanions === "yes" && (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mt-2 bg-muted/20 p-4 rounded-md border border-dashed border-border scale-in-center">
                        <div className="space-y-2">
                            <Label>{lang === 'pt' ? 'Nome do acompanhante:' : 'Companion Name:'}</Label>
                            <Input
                                {...register("companionName")}
                                onChange={(e) => setValue("companionName", e.target.value.replace(/[^a-zA-ZáéíóúàèìòùâêîôûãõçÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ\s]/g, ""))}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setValue("companionRelationship", e.target.value.replace(/[^a-zA-ZáéíóúàèìòùâêîôûãõçÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ\s]/g, ""))}
                            />
                        </div>
                    </div>
                )}
            ...
            <div className="space-y-3 border-t border-border pt-6">
                ...
                {isTravelingWithGroup === "yes" && (
                    <div className="mt-4 space-y-2 scale-in-center">
                        <Label htmlFor="groupName">{lang === 'pt' ? 'Nome do grupo:' : 'Group Name:'} *</Label>
                        <Input
                            id="groupName"
                            {...register("groupName")}
                        />
