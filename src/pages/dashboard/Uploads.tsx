...
export default function Uploads() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, docName: string) => {
    ...
    try {
      ...
      const filePath = `${user.id}/${docName.replace(/\s+/g, "_").toLowerCase()}_${Date.now()}.${fileExt}`;
      ...
      const { error: dbError } = await supabase
        ...
        }, { onConflict: "user_id,name" }); // Ajuste: talvez precise de uma constraint unique em user_id, name

      if (dbError) throw dbError;

      toast.success(lang === "pt" ? "Documento enviado com sucesso!" : "Document uploaded successfully!");
      fetchDocs();
    } catch (error: any) {
  ...
  const statusConfig = {
    ...
    approved: { label: lang === "pt" ? "Aprovado" : "Approved", icon: <CheckCircle2 className="h-4 w-4 text-green-500" />, color: "text-green-500" },
  };
    ...
    return (
      <div>
        ...
        <div className="mt-6 space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 shadow-card md:flex-row md:items-center md:justify-between">
        ...
        {expectedDocs.map((name, i) => {
          ...
          return (
            <div key={i} className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 shadow-card md:flex-row md:items-center md:justify-between">
              <div className="flex items-start gap-3 md:items-center">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-1.5 md:mt-0">
                    {isUploading ? <Loader2 className="h-3 w-3 animate-spin text-accent" /> : st.icon}
                    <span className={`text-xs font-medium ${st.color}`}>{isUploading ? (lang === "pt" ? "Enviando..." : "Uploading...") : st.label}</span>
                    {doc?.storage_path && !isUploading && <span className="truncate text-xs text-muted-foreground">— {doc.storage_path.split("/").pop()?.split("_")[0]}.{doc.storage_path.split(".").pop()}</span>}
                  </div>
                </div>
              </div>
              <Button
              ...
              >
                {isUploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UploadIcon className="mr-1 h-3.5 w-3.5" />}
