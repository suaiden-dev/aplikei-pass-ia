...
import { Check, Copy, Download, ExternalLink, Eye, Monitor, Save, Smartphone } from "lucide-react";
...
export default function PageBuilderPage() {
    ...
    const handleCopyUrl = () => {
        void navigator.clipboard.writeText(config.adminLawyerUrl).then(() => {
            setCopied(true);
...
<body>
  ...
  <section class="secterminal_panel::Toggle"ontainer">
      <h2>Como funciona</h2>
    ...
    return (
        <div className="flex h-full min-h-0 flex-col">
            <header className="flex items-center justify-between border-b border-border bg-card px-4 py-3">
                <div className="flex min-w-0 flex-1 flex-col gap-2 mr-4">
                    <div className="flex items-center gap-2">
                        <h1 className="text-lg font-black text-text">Landing Builder</h1>
                    </div>
                    <div className="flex items-center gap-1.5 rounded-lg border border-border bg-bg-subtle px-3 py-1.5 max-w-xl">
                        ...
                        <span className="shrink-0 rounded-full bg-info/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-info">
                            admin_lawyer
                        </span>
                        <button
                            type="button"
            ...
            <section className="flex h-[calc(100vh-64px)] min-h-0">
                ...
                <main className="flex-1 overflow-hidden bg-slate-300/70">
                    <div className="flex h-full items-start justify-center overflow-y-auto overflow-x-hidden">
                        <div style={{ zoom: previewViewport === "desktop" ? 0.48 : 1 }}>
                            <div
                                className={`mx-auto overflow-hidden bg-white shadow-2xl ${previewViewport === "desktop" ? "w-[1280px]" : "w-[390px]"
                                    }`}
                            >
