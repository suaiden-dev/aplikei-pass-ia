...
const getG1145Schema = (t: any) => z.object({
...
const getG1450Schema = (t: any) => z.object({
    ...
    <div className="space-y-8 pb-32">
      ...
      <div className="fixed bottom-0 left-0 lg:left-72 right-0 bg-white border-t border-slate-200 p-4 md:p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-40 flex items-center justify-between gap-4">
        ...
        >
          ...
          {t.cos.finalForms.btns.saveDraft}
        </button>
        ...
        >
          {isSubmitting ? (
            <><RiLoader4Line className="animate-spin text-lg" /> {t.cos.finalForms.btns.processing}</>
          ) : (
            <><RiCheckDoubleLine className="text-lg" /> {t.cos.finalForms.btns.submit} <RiArrowRightLine className="text-lg" /></>
          )}
        </button>
      </div>
    </div>
