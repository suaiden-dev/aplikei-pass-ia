...
export default function FinalPackageStep({ proc, onComplete, onJumpToStep }: Props) {
  ...
  const hasDependents = !!(data.dependents as unknown[])?.length || !!(data.paid_dependents);
  ...
  useEffect(() => {
    if (data.finalPackagePdfUrl) setMergedPdfUrl(data.finalPackagePdfUrl as string);
  ...
  useEffect(() => {
    if (!data.finalPackagePdfUrl) {
      setIsMerging(true);
      packageService.mergeAndUploadPackage(proc.id, proc.user_id!)
  ...
  const packageOrder: { name: string; ready: boolean; note?: string }[] = [
    ...
    { name: "Formulário I-539 — Aplicante Principal", ready: !!data.i539PdfUrl },
    ...
    { name: "I-20 F2 (Dependentes)", ready: hasDependents && !!docs.i20_document },
    ...
    { name: "Passaporte e Visto — Aplicante Principal", ready: !!docs.passportVisa },
  ...
  if (showCelebration) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-8 animate-in fade-in zoom-in-95 duration-1000">
        ...
        <p className="text-slate-500 font-medium max-w-md mx-auto leading-relaxed mb-12" dangerouslySetInnerHTML={{ __html: t.cos.finalPackage.celebration.desc }} />
        <div className="bg-white rounded-[40px] border border-slate-100 p-12 shadow-xl shadow-slate-200/50 mb-12 max-w-lg w-full relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm text-slate-400 font-medium leading-relaxed">{t.cos.finalPackage.celebration.nextStepInfo}</p>
          </div>
        </div>
        <button
          onClick={async () => {
    ...
    <div className="space-y-8 pb-32">
      ...
      {deadline && (
        <div className="bg-red-50 border border-red-100 rounded-[24px] p-6 flex items-start gap-4">
          ...
          <div>
            <p className="text-[11px] font-black text-red-600 uppercase tracking-widest mb-1">Data Limite de Envio</p>
            <p className="text-xl font-black text-red-700">{deadline}</p>
            <p className="text-[11px] text-red-500 font-bold mt-1 uppercase tracking-wide">1 semana antes da data de permanência autorizada do I-94</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 p-10 shadow-sm text-center">
        ...
        <h3 className="text-xl font-black text-slate-800 mb-2 uppercase tracking-tight">{t.cos.finalPackage.download.title}</h3>
        <p className="text-sm text-slate-400 font-medium max-w-sm mx-auto leading-relaxed mb-8">
          Faça o download do seu pacote e realize as assinaturas conforme instruções abaixo.
        </p>
        {isMerging ? (
          <div className="flex items-center justify-center gap-3 py-5 text-slate-400">
            ...
            <span className="text-sm font-black uppercase tracking-widest">{t.cos.finalPackage.download.btnWaiting}</span>
          </div>
        ) : (
          <button
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Montar o Processo</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Organize nesta ordem exata antes de enviar</p>
          </div>
        </div>
        <div className="space-y-2">
          {packageOrder.map((item, i) => (
            <div key={i} className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${item.ready ? "bg-emerald-50/50 border-emerald-100" : "bg-slate-50 border-slate-100"}`}>
              <span className={`text-[10px] font-black w-5 text-center shrink-0 ${item.ready ? "text-emerald-500" : "text-slate-300"}`}>{i + 1}</span>
              {item.ready
                ? <RiCheckboxCircleFill className="text-emerald-500 text-base shrink-0" />
                : <div className="w-4 h-4 rounded-full border-2 border-slate-200 shrink-0" />
              }
              <span className={`text-[11px] font-black uppercase tracking-widest ${item.ready ? "text-slate-700" : "text-slate-400"}`}>{item.name}</span>
            </div>
          ))}
        </div>
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-5 text-center">
          Gere o PDF único clicando em Download acima — todos os documentos já estarão nesta ordem.
        </p>
      </div>
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Assinaturas Necessárias</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Use caneta preta — assinaturas à mão</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
            <p className="text-[11px] font-black text-slate-800 uppercase tracking-widest mb-1">G-1450</p>
            <p className="text-[11px] text-slate-500 font-medium">Assine no campo destinado ao titular do cartão.</p>
          </div>

          {/* Cover Letter */}
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
            <p className="text-[11px] font-black text-slate-800 uppercase tracking-widest mb-1">Cover Letter (Carta de Apresentação)</p>
            <p className="text-[11px] text-slate-500 font-medium">Assine no campo do aplicante principal ao final da carta.</p>
          </div>

          {/* I-539 */}
          <div className="p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
            <p className="text-[11px] font-black text-blue-700 uppercase tracking-widest mb-2">Formulário I-539 — Aplicante Principal</p>
            <p className="text-[11px] text-slate-600 font-medium">📄 Pág 5, Parte 5, Item 4 — Assine com seu nome completo.</p>
          </div>

          {/* I-539A */}
          <div className="p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
            <p className="text-[11px] font-black text-blue-700 uppercase tracking-widest mb-2">Formulário I-539A — Dependentes</p>
            <p className="text-[11px] text-slate-600 font-medium mb-2">📄 Pág 3, Parte 4, Item 4</p>
            <div className="space-y-1.5 pl-3 border-l-2 border-blue-200">
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide">⬇ Menores de 14 anos → assinado pelo aplicante principal</p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide">⬇ Maiores de 14 anos → o próprio dependente assina</p>
            </div>
          </div>

          {/* I-20 */}
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
            <p className="text-[11px] font-black text-slate-800 uppercase tracking-widest mb-1">I-20 F1 e F2</p>
            <p className="text-[11px] text-slate-500 font-medium">📄 Pág 1 — Sempre assinado pelo aplicante principal.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Endereço de Envio (USCIS)</h3>
        </div>
        <div className="bg-slate-50 rounded-2xl border border-slate-100 p-6 mb-4">
          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3">Destinatário</p>
          <p className="font-mono text-sm text-slate-700 leading-relaxed">U.S. Department of Homeland Security</p>
          <p className="font-mono text-sm text-slate-700 leading-relaxed">2501 S. State Highway 121 Business</p>
          <p className="font-mono text-sm text-slate-700 leading-relaxed">Suite 400</p>
          <p className="font-mono text-sm text-slate-700 leading-relaxed">Lewisville, TX 75067</p>
        </div>
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-2xl p-4">
          ...
          <p className="text-[11px] text-amber-700 font-bold uppercase tracking-wide">
            Sugestão: enviar via FedEx com entrega em 24h (express delivery) com número de rastreamento.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Código de Rastreamento</h3>
        </div>
        <input
          type="text"
          value={trackingCode}
          onChange={e => { setTrackingCode(e.target.value); trackingSavedRef.current = false; }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
          <button
            onClick={async () => {
              ...
              await processService.updateStepData(proc.id, { uscis_official_result: "approved", uscis_reported_at: new Date().toISOString() });
          ...
          >
            ...
            <span className="text-[9px] font-black text-slate-700 uppercase tracking-widest text-center leading-tight">{t.cos.finalPackage.feedback.rfe}</span>
          </button>
        </div>
      </div>
