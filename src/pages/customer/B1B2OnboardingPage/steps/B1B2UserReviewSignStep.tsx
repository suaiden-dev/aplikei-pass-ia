...
export function B1B2UserReviewSignStep({ procId, userId, stepData, onComplete, onBack }: B1B2UserReviewSignStepProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* ── Tutorial Carousel ── */}
      <div className="bg-card rounded-[32px] border border-border shadow-xl shadow-none overflow-hidden">
        <div className="p-8 pb-4 border-b border-border items-center justify-between">
          <h3 className="font-display font-black text-2xl text-text">
        ...
        {/* Credentials */}
        <div className="bg-bg-subtle p-6 md:p-8 flex flex-col md:flex-row gap-6">
           <div className="basis-1/3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl font-black text-text uppercase tracking-wider">{appId}</span>
                <button onClick={() => handleCopy(appId)} className="p-1.5 bg-card border border-border rounded-lg text-text-muted hover:text-primary transition-all"><RiFileCopyLine /></button>
              </div>
           </div>
           <div className="basis-1/3 border-l border-border pl-6">
              <span className="block text-[10px] font-black text-text-muted uppercase tracking-widest mb-1.5">
                {t.onboardingPage.motherSecurity}
              </span>
              <div className="flex items-center gap-2">
                <span className="text-xl font-black text-text uppercase tracking-wider">{motherName}</span>
                <button onClick={() => handleCopy(motherName)} className="p-1.5 bg-card border border-border rounded-lg text-text-muted hover:text-primary transition-all"><RiFileCopyLine /></button>
              </div>
           </div>
           <div className="basis-1/3 border-l border-border pl-6">
              <span className="block text-[10px] font-black text-text-muted uppercase tracking-widest mb-1.5">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl font-black text-text tracking-wider">{birthDate}</span>
                <button onClick={() => handleCopy(birthDate)} className="p-1.5 bg-card border border-border rounded-lg text-text-muted hover:text-primary transition-all"><RiFileCopyLine /></button>
              </div>
           </div>
        </div>
        ...
        <div className="p-6 md:p-8">
           ...
           <div className="text-center mb-8 min-h-[60px]">
             ...
             <p className="text-sm font-medium text-text-muted mt-2 max-w-xl mx-auto">{tutorialSteps[activeStep].desc}</p>
           </div>
      ...
      {/* ── File Uploads ── */}
      <h3 className="font-display font-black text-xl text-text mt-12 mb-6 text-center">
      ...
      <div className="pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            ...
            className="w-full sm:w-auto px-6 py-3.5 rounded-xl border border-border text-text-muted font-bold text-xs uppercase tracking-widest hover:bg-bg-subtle transition-all"
          >
