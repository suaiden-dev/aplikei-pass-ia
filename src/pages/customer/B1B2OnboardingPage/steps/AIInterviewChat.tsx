...
export function AIInterviewChat({ onBack, serviceId }: AIInterviewChatProps) {
  ...
  const [ds160Data, setDs160Data] = useState<any>(null); // To store DS-160 for context
  ...
  useEffect(() => {
    if (serviceId) {
       supabase
         ...
         .single()
         .then(({ data }) => {
            if (data?.step_data) setDs160Data(data.step_data);
         });
    }
  }, [serviceId]);
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_N8N_BOT_INTERVIEW, {
        ...
        body: JSON.stringify({
          ...
          lang: lang,
          sessionId: sessionId,
          ds160: ds160Data // Passing context to chatbot
        }),
      });
