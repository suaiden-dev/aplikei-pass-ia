    ...
    <div className="py-16">
      <div className="container max-w-4xl">
        ...
        <div className="relative mt-16">
          ...
          {p.steps.map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className={`relative mb-6 flex items-start gap-4 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
              ...
              <div className={`ml-16 max-w-sm rounded-md border border-border bg-card p-5 shadow-card md:ml-0 ${i % 2 === 0 ? "md:mr-auto md:pr-16" : "md:ml-auto md:pl-16"}`}>
        ...
        <div className="mt-16 grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={i} className={`rounded-md border p-4 shadow-card ${card.highlight ? "border-accent bg-accent/5" : "border-border bg-card"}`}>
              <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-md ${card.highlight ? "bg-accent text-accent-foreground" : "bg-primary text-primary-foreground"}`}>
                {card.icon}
              </div>
        ...
        {/* What AI does */}
        <div className="mt-16 rounded-md border border-border bg-card p-5 shadow-card">
          <h2 className="font-display text-title font-bold text-foreground">{p.aiDoesTitle[lang]}</h2>
