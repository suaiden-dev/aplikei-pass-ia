...
export default function RevenuePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (tab === "approved_payments") {
      ...
      const { data: approvedZelle } = await supabase
        .from("zelle_payments")
        .select("*, offices(name)")
      ...
      (approvedZelle ?? []).forEach((r: any) => {
        results.push({
          ...
          clientName: r.guest_name ?? "Guest",
          clientEmail: r.guest_email ?? "",
          serviceName: r.service_slug?.replace(/-/g, " ").toUpperCase() || "GENERAL",
          serviceSlug: r.service_slug || "",
          amount: Number(r.amount) || 0,
          method: "ZELLE",
          createdAt: r.created_at,
    ...
    <div className="p-4 sm:p-8 space-y-8 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      ...
      <div className="flex items-end gap-8 border-b border-border">
        ...
        {canAccessOfficeRequests && (
          ...
          >
            {t.payments.tabs.officeRequests}
      ...
      <AnimatePresence>
        {selectedPayment && (
          <DetailModal 
            ...
            onApprove={selectedPayment.source === 'zelle' && canManageZelleVerification ? () => handleApproveZelle(selectedPayment) : undefined}
            onReject={selectedPayment.source === 'zelle' && canManageZelleVerification ? () => handleUpdateStatus(selectedPayment.id, 'rejected') : undefined}
            busy={!!busy}
          />
        )}
      </AnimatePresence>
