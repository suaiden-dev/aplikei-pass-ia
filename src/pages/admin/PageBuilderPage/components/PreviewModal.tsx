...
export function PreviewModal({ open, config, onOpenChange }: PreviewModalProps) {
  const [viewport, setViewport] = React.useState<ViewportMode>("desktop");

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-screen w-screen max-w-none gap-0 rounded-none border-none p-0" showClose={false}>
        <DialogHeader className="flex-row items-center justify-between border-b border-border bg-card px-4 py-3">
          <DialogTitle className="text-base font-black">Preview da Landing</DialogTitle>
        ...
        <div className="flex h-[calc(100vh-64px)] overflow-auto bg-slate-200 p-6">
          <div className={`mx-auto min-h-full overflow-hidden rounded-2xl bg-white shadow-2xl ${viewportClass[viewport]}`}>
