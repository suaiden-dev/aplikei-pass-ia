    ...
    <div className="min-h-full overflow-x-hidden bg-slate-950 text-slate-100">
      <header className="sticky top-0 z-10 border-b border-slate-800/80 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:h-20 md:px-6">
          <img src={config.logoUrl} alt="Logo" className="h-8 w-auto object-contain md:h-10" />
          <div className="flex items-center gap-2">
            <a href={config.loginUrl}>
              <Button variant="outline" className="h-10 rounded-full border-slate-700 px-4 text-xs text-slate-100 hover:bg-slate-900 md:h-11 md:px-6 md:text-sm">
      ...
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.28),_rgba(2,6,23,1)_42%)] px-4 py-14 md:px-6 md:py-24">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-8 md:gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <h1 className="mt-5 text-3xl font-black leading-tight text-white sm:text-4xl md:text-6xl">{config.heroTitle}</h1>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base md:mt-6 md:text-lg">{config.heroSubtitle}</p>
            <div className="mt-8 flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-row sm:items-center">
              <a href={config.primaryCtaUrl}>
                <Button className="h-12 w-full rounded-full bg-white px-8 text-sm font-bold text-slate-900 hover:bg-slate-200 sm:w-auto">{config.primaryCtaLabel}</Button>
              </a>
              <a href={config.secondaryCtaUrl}>
                <Button variant="outline" className="h-12 w-full rounded-full border-slate-500 px-8 text-sm font-bold text-slate-100 hover:bg-slate-900 sm:w-auto">
          ...
          <div className="rounded-3xl border border-slate-700/60 bg-slate-900/70 p-5 shadow-[0_20px_70px_rgba(8,145,178,0.25)] sm:p-6">
            <p className="text-sm font-semibold text-cyan-300">Atendimento com advogado responsável</p>
            <p className="mt-2 text-2xl font-black text-white">{config.lawyerName}</p>
            <p className="mt-4 text-sm leading-relaxed text-slate-300">{config.lawyerCtaText}</p>
      ...
      <section className="bg-white px-4 py-14 text-slate-900 md:px-6 md:py-20">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-center text-2xl font-black md:text-4xl">Serviços de vistos com foco em resultado</h2>
          <p className="mx-auto mt-3 max-w-3xl text-center text-slate-600">
            Soluções jurídicas para cada etapa da sua jornada imigratória, com organização documental e estratégia personalizada.
          </p>
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2">
            {[
              { title: "Visto B1/B2", text: "Turismo e negócios com preparação de perfil, DS-160 e orientação de entrevista." },
              { title: "Visto F1", text: "Plano completo para estudantes, alinhando documentação acadêmica e narrativa migratória." },
              { title: "Extensão de Status", text: "Solicitação técnica para ampliar permanência regular sem improviso." },
              { title: "Troca de Status", text: "Mudança de categoria com estratégia jurídica e mitigação de riscos de negação." },
            ].map((service) => (
      ...
      <section className="bg-slate-100 px-4 py-14 text-slate-900 md:px-6 md:py-20">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-center text-2xl font-black md:text-4xl">Como funciona</h2>
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              { step: "01", title: "Diagnóstico", text: "Análise do seu histórico e do objetivo migratório." },
              { step: "02", title: "Plano Jurídico", text: "Definição de estratégia e checklist documental." },
              { step: "03", title: "Execução", text: "Preenchimento, revisão e acompanhamento até o protocolo." },
            ].map((item) => (
      ...
      <section className="bg-slate-950 px-4 py-14 md:px-6 md:py-20">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-center text-2xl font-black text-white md:text-4xl">Quem contrata recomenda</h2>
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              ...
              <blockquote key={quote} className="rounded-2xl border border-slate-800 bg-slate-900 p-6 text-sm text-slate-300">
                {quote}
              </blockquote>
            ))}
          </div>
      ...
      <section className="bg-white px-4 py-14 text-slate-900 md:px-6 md:py-20">
        <div className="mx-auto max-w-4xl">
          <h2 className="text-center text-2xl font-black md:text-4xl">Dúvidas frequentes</h2>
      ...
      <section className="bg-gradient-to-r from-cyan-600 to-blue-700 px-4 py-14 text-white md:px-6 md:py-16">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 text-center md:flex-row md:items-start md:text-left">
          <div>
            <h2 className="text-2xl font-black md:text-3xl">Pronto para estruturar seu pedido de visto com segurança?</h2>
            <p className="mt-2 text-cyan-100">Fale com nossa equipe e receba um plano inicial para o seu caso.</p>
          </div>
          <div className="flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-row sm:items-center">
            <a href={config.primaryCtaUrl}>
              <Button className="h-12 w-full rounded-full bg-white px-8 font-bold text-slate-900 hover:bg-slate-100 sm:w-auto">{config.primaryCtaLabel}</Button>
            </a>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-800 bg-slate-950 px-6 py-8 text-center text-xs text-slate-400">
        © {new Date().getFullYear()} {config.lawyerName}. Todos os direitos reservados.
      </footer>
    </div>
  );
}
