        ...
        try {
            ...
            if (srIds.length > 0) {
                const { data: ordersData } = await supabase
                    .from("visa_orders")
                    .select("id, order_number, product_slug, total_price_usd, payment_status, created_at")
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <Card>
                            ...
                            <CardContent className="grid gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    <p className="text-xs text-muted-foreground uppercase">Nacionalidade</p>
                                    <p className="font-medium">{client.nationality || "—"}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-muted-foreground uppercase">{client.document_type || 'Documento'}</p>
                                    <p className="font-medium">{client.document_number || "—"}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-muted-foreground uppercase">Estado Civil</p>
                                    <p className="font-medium capitalize">{client.marital_status || "—"}</p>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent className="grid gap-4 sm:grid-cols-2">
                                <div className="sm:col-span-2">
                                    <p className="text-xs text-muted-foreground uppercase">Rua / Complemento</p>
                                    <p className="font-medium">{client.address_line || "—"}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-muted-foreground uppercase">Cidade</p>
                                    <p className="font-medium">{client.city || "—"}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-muted-foreground uppercase">Estado/Província</p>
                ...
                <TabsContent value="pedidos" className="mt-6">
                    <Card>
                        <AdminDataTable
                            ...
                            data={orders}
                            columns={[
                                { key: "order_number", header: "Nº Pedido", className: "font-mono" },
                                {
                                    key: "product_slug",
                                    header: "Produto",
                                    render: (item) => <Badge variant="secondary" className="capitalize">{item.product_slug.replace(/-/g, " ")}</Badge>
                                },
                                { key: "total_price_usd", header: "Valor", render: (item) => formatCurrency(item.total_price_usd) },
                                {
                                    key: "payment_status",
                                    header: "Status",
                                    render: (item) => (
                                ...
                                { key: "created_at", header: "Data", render: (item) => formatDate(item.created_at) },
                                {
                                    key: "actions",
                                    header: "",
                                    className: "text-right",
                                    render: (item) => (
                                        <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/pedidos/${item.id}`)}>
                ...
                <TabsContent value="documentos" className="mt-6">
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {files.length === 0 ? (
                            ...
                            files.map((file) => (
                                <Card key={file.id} className="overflow-hidden group hover:border-primary transition-colors">
                                    <CardContent className="p-4 flex items-start justify-between gap-4">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="space-y-1">
                                                <p className="text-sm font-semibold line-clamp-1">{file.file_name || 'Documento'}</p>
                                                <p className="text-xs text-muted-foreground capitalize">{file.file_type.replace(/_/g, " ")}</p>
