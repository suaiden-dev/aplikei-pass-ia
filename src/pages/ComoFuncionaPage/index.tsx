...
export default function ComoFuncionaPage() {
  ...
  return (
    <div className="bg-bg">
      {/* Page Hero */}
      <section className="pt-20 lg:pt-32 pb-16 lg:pb-24 px-8 lg:px-16 bg-bg overflow-hidden">
          ...
          >
            <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
              ...
              <h1 className="text-5xl lg:text-7xl font-black text-primary leading-[1.05] mb-10 tracking-tighter">
                {p.hero.title} <span className="text-text opacity-20">{p.hero.titleHighlight}</span>
              </h1>
              
              <p className="text-lg lg:text-2xl text-text-muted opacity-80 font-medium leading-relaxed mb-12 italic">
                {p.hero.description}
              </p>
              ...
              <div className="flex flex-col gap-6 items-center lg:items-start text-left">
                {p.hero.features.map((item: string, i: number) => (
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <span className="text-text font-bold text-sm tracking-tight">{item}</span>
                  </div>
                ))}
              </div>
            </div>
