...
import { Loader2, FileText, ChevronLeft, ChevronRight, AlertCircle } from "lucide-react";
...
export function AdminDS160Viewer({ userId, clientName, isOpen, onClose }: AdminDS160ViewerProps) {
    ...
    const fetchDS160Data = async () => {
        ...
        try {
            ...
            if (serviceError) {
                console.error("❌ [AdminDS160Viewer] Erro ao buscar serviço:", serviceError);
            }

            const service = services?.[0];
            console.log("📄 [AdminDS160Viewer] Serviço (mais recente) encontrado:", service);

            if (!service) {
                console.warn("⚠️ [AdminDS160Viewer] Nenhum serviço 'visto-b1-b2' encontrado para este usuário.");
                setFormData(null);
                setLoading(false);
                return;
            }
            ...
            console.log("📦 [AdminDS160Viewer] Respostas brutas do banco:", responses);

            if (responses && responses.length > 0) {
                const combined = responses.reduce((acc, curr) => ({ ...acc, ...(curr.data as any) }), {});
                console.log("✅ [AdminDS160Viewer] Dados combinados com sucesso:", combined);
                setFormData(combined);
            } else {
                console.warn("⚠️ [AdminDS160Viewer] O serviço existe, mas não há respostas na tabela onboarding_responses.");
                setFormData(null);
            }
        } catch (err) {
    ...
    const sections = formData ? [
        ...
        ...s,
        fields: s.fields.filter(f => f.value && f.value !== '—' && f.value !== 'undefined undefined')
                ...
                <div className="flex-1 overflow-y-auto py-4 min-h-[300px]">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
                            ...
                            <p className="text-muted-foreground">Nenhum dado do DS-160 encontrado para este usuário.</p>
                        </div>
                    ) : (
                        <div className="space-y-6">
                            <div className="flex items-center justify-between border-b pb-2">
                                <h3 className="font-bold text-lg text-primary">{currentSection?.title}</h3>
                                <Badge variant="secondary">{currentSectionIdx + 1} de {sections.length}</Badge>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                                {currentSection?.fields.map((field, i) => (
                                    <div key={i} className="space-y-1">
                                        ...
                                        <p className="text-sm font-semibold text-foreground bg-muted/30 p-2 rounded-md border border-border/40">{field.value}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
