...
export function NotificationBell() {
  ...
  useEffect(() => {
    ...
    // Note: RLS policies on the 'notifications' table ensure we only receive
    // events that we are authorized to see (our own or admin ones if we are admin).
    const uniqueChannel = `notifications-${user.id}-${Math.random().toString(36).slice(2, 9)}`;
