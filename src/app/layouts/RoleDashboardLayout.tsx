...
import { useT, useLocale, type Language } from "@app/app/i18n";
...
import { useOfficeBranding } from "@shared/hooks/useOfficeBranding";
...
export function RoleDashboardLayout({
  ...
  const { companyName, logoUrl, isWhiteLabel, loading: isBrandLoading } = useOfficeBranding({
            ...
            <div className="flex items-center gap-3">
              {isWhiteLabel ? (
                <div className="rounded-2xl bg-bg-subtle p-2 text-text shrink-0 border border-border">
                  {isBrandLoading ? (
                    <div className="h-6 w-6 rounded-md bg-slate-200/70 animate-pulse" />
                  ) : (
