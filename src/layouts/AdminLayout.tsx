  ...
  return (
    <NotificationProvider role="admin">
      <div className="flex h-screen overflow-hidden bg-[#F8FAFC] relative">
        ...
        {isSidebarOpen && (
          <div 
            className="xl:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 transition-opacity"
        ...
        <aside
          className={cn(
            "fixed xl:static inset-y-0 left-0 z-[60] flex flex-col shrink-0 w-full xl:w-[220px] bg-highlight text-white transition-transform duration-300 xl:translate-x-0 shadow-2xl xl:shadow-none",
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          {/* Top bar */}
          <header className="flex items-center justify-between px-6 xl:px-8 py-4 bg-white border-b border-slate-100 shrink-0">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setIsSidebarOpen(true)}
                className="xl:hidden p-2 rounded-xl bg-slate-50 text-slate-600 active:scale-95 transition-all"
              >
                <RiMenuLine size={24} />
              </button>
              <h1 className="font-display font-semibold text-lg xl:text-xl text-slate-800">{title}</h1>
            </div>
