import { ArrowDown, ArrowUp, Check, Copy, HelpCircle, ImageUp, Palette, LayoutTemplate, Layers, MessageSquare, Link as LinkIcon, Settings2, LogOut } from "lucide-react";
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <p className="pt-4 pb-1 text-[11px] font-semibold uppercase tracking-widest text-text-muted border-t border-border/50 first:border-t-0 mt-4">
      {children}
    </p>
  );
}
...
const THEME_PRESETS: Array<{
  ...
  {
    name: "Aplikei",
    description: "Clean classic blue.",
    values: {
  ...
  {
    name: "Premium Green",
    description: "Modern green, builds trust.",
    values: {
...
export function BuilderSidebar({
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <aside className="flex h-full shrink-0 border-l border-border bg-background z-20 overflow-hidden">
        {/* Navigation Tab List */}
        <div className="w-20 shrink-0 flex flex-col items-center justify-between border-r border-border bg-card py-4">
           <div className="flex flex-col items-center gap-2">
             <button onClick={() => setActiveTab("branding")} className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-colors ${activeTab === "branding" ? "text-primary bg-primary/10" : "text-text-muted hover:bg-bg-subtle"}`}><Palette size={20}/><span className="text-[10px] font-bold text-center">Appearance</span></button>
             <button onClick={() => setActiveTab("hero")} className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-colors ${activeTab === "hero" ? "text-primary bg-primary/10" : "text-text-muted hover:bg-bg-subtle"}`}><LayoutTemplate size={20}/><span className="text-[10px] font-bold text-center">Top Section</span></button>
             <button onClick={() => setActiveTab("services")} className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-colors ${activeTab === "services" ? "text-primary bg-primary/10" : "text-text-muted hover:bg-bg-subtle"}`}><Layers size={20}/><span className="text-[10px] font-bold text-center">Services</span></button>
             <button onClick={() => setActiveTab("proof")} className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-colors ${activeTab === "proof" ? "text-primary bg-primary/10" : "text-text-muted hover:bg-bg-subtle"}`}><MessageSquare size={20}/><span className="text-[10px] font-bold text-center">Proof</span></button>
        ...
        {/* Editor Panel */}
        <div className="w-[340px] shrink-0 overflow-y-auto p-4 bg-background">
           ...
           {activeTab === "branding" && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor="page-title">Website Title (Browser Tab)</Label>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor="favicon-upload">Browser Icon (Favicon)</Label>
                  ...
                  <div className="rounded-xl border border-dashed border-border bg-bg-subtle p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        <p className="truncate text-xs text-text-muted">
                          {isUploadingFavicon ? "Uploading favicon..." : "PNG, JPG, or SVG."}
                        </p>
                      </div>
                      <label
                  ...
                  <div className="rounded-xl border border-dashed border-border bg-bg-subtle p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        <p className="truncate text-xs text-text-muted">
                          {isUploadingLogo ? "Uploading logo..." : "PNG, JPG, or SVG."}
                        </p>
                      </div>
                      <label
                        htmlFor="logo-upload"
                ...
                <SectionHeading>Website Colors</SectionHeading>
                <div className="space-y-2 rounded-lg border border-border bg-background p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-xs font-semibold text-text">Color Presets</p>
                    <button
                      type="button"
                      onClick={resetTheme}
                  ...
                  <div className="rounded-lg border border-warning/30 bg-warning/10 p-3 text-xs text-text">
                    ...
                    <ul className="mt-2 space-y-1 text-text-muted">
                      {contrastIssues.map((issue) => (
                        <li key={issue.label}>
                          {issue.label}: {issue.ratio?.toFixed(2)}:1. Min 4.5:1.
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <SectionHeading>Light Mode Colors</SectionHeading>
                <div className="space-y-3">
                  {colorField("light-primary-color", "Main Accent Color", config.lightPrimaryColor, (next) => onUpdateConfig("lightPrimaryColor", next))}
                  ...
                  {colorField("light-surface-color", "Panel Background", config.lightSurfaceColor, (next) => onUpdateConfig("lightSurfaceColor", next))}
                  {colorField("light-text-color", "Primary Text", config.lightTextColor, (next) => onUpdateConfig("lightTextColor", next))}
                  {colorField("light-muted-text-color", "Secondary Text", config.lightMutedTextColor, (next) => onUpdateConfig("lightMutedTextColor", next))}
                </div>

                <SectionHeading>Dark Mode Colors</SectionHeading>
                <div className="space-y-3">
                  {colorField("dark-primary-color", "Main Accent Color", config.darkPrimaryColor, (next) => onUpdateConfig("darkPrimaryColor", next))}
                  {colorField("dark-background-color", "Page Background", config.darkBackgroundColor, (next) => onUpdateConfig("darkBackgroundColor", next))}
                  {colorField("dark-surface-color", "Panel Background", config.darkSurfaceColor, (next) => onUpdateConfig("darkSurfaceColor", next))}
                  {colorField("dark-text-color", "Primary Text", config.darkTextColor, (next) => onUpdateConfig("darkTextColor", next))}
                  {colorField("dark-muted-text-color", "Secondary Text", config.darkMutedTextColor, (next) => onUpdateConfig("darkMutedTextColor", next))}
                </div>

                <SectionHeading>Element Shapes</SectionHeading>
                <div className="space-y-3">
                  {colorField("primary-button-color", "Main Button Color", config.primaryButtonColor, (next) => onUpdateConfig("primaryButtonColor", next))}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="card-radius">Panel Corner Radius</Label>
                    <Input
                      id="card-radius"
                      type="number"
                      min="0"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="card-shadow-style">Panel Shadow Style</Label>
                    <select
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="button-radius-style">Button Style</Label>
                    <select
           ...
           {activeTab === "hero" && (
              <div className="space-y-4">
                <SectionHeading>Hero text</SectionHeading>
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                ...
                <SectionHeading>Client Portal Preview Image</SectionHeading>
                <div className="space-y-2">
                ...
                <SectionHeading>Portal checklist and docs</SectionHeading>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="lawyer-name">Your Name</Label>
                  <Input
                    id="lawyer-name"
           ...
           {activeTab === "services" && (
              <div className="space-y-4">
                ...
                <div className="space-y-2 border-t border-border/50 pt-3">
                  {toggleField("service-b1-enabled", "Show B1/B2 Option", config.serviceB1B2Enabled || false, (next) => onUpdateConfig("serviceB1B2Enabled", next))}
                ...
                <div className="space-y-2 border-t border-border/50 pt-3">
                  {toggleField("service-f1-enabled", "Show F1 Option", config.serviceF1Enabled || false, (next) => onUpdateConfig("serviceF1Enabled", next))}
                ...
                <div className="space-y-2 border-t border-border/50 pt-3">
                  {toggleField("service-eos-enabled", "Show Extension Option (EOS)", config.serviceEOSEnabled || false, (next) => onUpdateConfig("serviceEOSEnabled", next))}
                ...
                <div className="space-y-2 border-t border-border/50 pt-3">
                  {toggleField("service-cos-enabled", "Show Change of Status (COS)", config.serviceCOSEnabled || false, (next) => onUpdateConfig("serviceCOSEnabled", next))}
           ...
           {activeTab === "links" && (
              <div className="space-y-4">
                <SectionHeading>Links & Buttons</SectionHeading>
                {urlIssues.length ? (
                  <div className="rounded-lg border border-warning/30 bg-warning/10 p-3 text-xs text-text mb-4">
                ...
                {linkField(
                  ...
                  undefined,
                  "Redirect link to your master admin dashboard.",
                )}
                ...
                {linkField(
                  ...
                  undefined,
                  undefined,
                  "Link to direct users when they click Login.",
                )}
                
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor="login-label">Login Button Text</Label>
                    <FieldTooltip content="Text displayed on the login button in the header." />
                  </div>
                  <Input
                    id="login-label"
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor="primary-cta-label">Main Button Text</Label>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor="secondary-cta-label">Secondary Button Text</Label>
                ...
                <SectionHeading>Footer Links</SectionHeading>
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
           ...
           {activeTab === "structure" && (
              <div className="space-y-4">
                ...
                <SectionHeading>Reorder Sections</SectionHeading>
                <div className="space-y-2">
