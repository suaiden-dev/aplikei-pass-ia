...
export function useRevenuePage() {
  ...
  const [pageByTab, setPageByTab] = useState<Record<RevenueTab, number>>({
    zelle: 1,
    office_requests: 1,
    approved_payments: 1,
  });
  ...
  const load = useCallback(async () => {
    ...
    if (tab === "office_requests") {
      ...
      if (withdrawalOfficeIds.length > 0) {
        ...
        payoutSettingsData.forEach((s) => {
          if (s?.office_id) {
            payoutSettingsByOfficeId.set(s.office_id, {
              zelle_name: s.zelle_name ?? null,
              zelle_identifier: s.zelle_identifier ?? null,
            });
          }
        });
      }
    ...
    if (tab === "approved_payments") {
      ...
      if (ownerIds.length > 0) {
        ...
        inferred.forEach((value, key) => inferredOfficeByOwnerId.set(key, value));
      }
  ...
  const handleApproveZelle = async (p: UnifiedPayment) => {
    ...
    setBusy(p.id);
    try {
      await paymentService.approveZellePayment(p.zelleId, user?.fullName || user?.email || "Admin");
  ...
  const handleUpdateWithdrawalStatus = async (id: string, status: "approved" | "rejected") => {
    ...
    try {
      ...
      const message = detail || (err as { message?: string })?.message || "";
      toast.error(
        message
