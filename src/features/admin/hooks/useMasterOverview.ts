...
export function useMasterOverview() {
  ...
  const statsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: zellePayments }, { data: orders }] = await Promise.all([
        supabase.from("zelle_payments").select("amount, status"),
        supabase.from("orders").select("total_price_usd, payment_status"),
      ]);

      const approvedZelleTotal = (zellePayments || [])
        .filter((p) => ["approved", "paid", "completed"].includes(String(p.status || "").toLowerCase()))
      ...
      return {
        revenueTotal: approvedZelleTotal + paidOrdersTotal,
  ...
  const monthlyRevenueQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      (zellePayments || [])
        .filter((p) => ["approved", "paid", "completed"].includes(String(p.status || "").toLowerCase()))
        .forEach((p) => {
          const mKey = new Date(p.created_at).toLocaleString(localeCode, { month: "short" }).replace(".", "");
          if (monthlyMap[mKey] !== undefined) monthlyMap[mKey] += Number(p.amount) || 0;
        });
  ...
  const distributionQuery = useQuery({
    ...
    queryFn: async () => {
      const [{ data: orders }, { data: zellePayments }] = await Promise.all([
        supabase.from("orders").select("product_slug, payment_status, office_id"),
        supabase.from("zelle_payments").select("service_slug, status, office_id"),
      ]);
      ...
      (zellePayments || [])
        .filter((payment) => ["approved", "paid", "completed"].includes(String(payment.status || "").trim().toLowerCase()))
        .forEach((payment) => {
          const slug = String(payment.service_slug || "").trim();
          if (!slug) {
            dist.Others += 1;
            return;
          }
