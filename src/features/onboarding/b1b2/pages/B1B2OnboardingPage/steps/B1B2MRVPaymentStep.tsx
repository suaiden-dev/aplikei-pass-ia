  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await notificationService.notifyAdmin({
        ...
        category: "b1b2",
        action: "mrv_payment_confirmed",
        metadata: { payment_method: method === "credit_card" ? "Credit card" : "Bank slip" },
      });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-black text-text tracking-tight">{t.onboardingPage.paymentPending.mrvFeeTitle}</h2>
        <p className="text-sm font-medium text-text-muted mt-2">{t.onboardingPage.paymentPending.mrvFeeDesc}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button
          ...
          className={`p-6 rounded-[28px] border-2 transition-all text-left flex flex-col gap-4 ${
            method === "credit_card" ? "border-primary bg-primary/5 shadow-lg shadow-primary/10" : "border-border bg-card hover:border-border/80"
          }`}
        >
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${method === "credit_card" ? "bg-primary text-white" : "bg-bg-subtle text-text-muted"}`}>
            <RiBankCard2Line className="text-2xl" />
          </div>
          <div>
            <h3 className="font-black text-text text-sm uppercase tracking-widest">{t.onboardingPage.paymentPending.creditCard}</h3>
            <p className="text-[10px] text-text-muted font-bold uppercase mt-1">{t.onboardingPage.paymentPending.immediatePayment}</p>
          </div>
        ...
        <button
          ...
          className={`p-6 rounded-[28px] border-2 transition-all text-left flex flex-col gap-4 ${
            method === "boleto" ? "border-primary bg-primary/5 shadow-lg shadow-primary/10" : "border-border bg-card hover:border-border/80"
          }`}
        >
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${method === "boleto" ? "bg-primary text-white" : "bg-bg-subtle text-text-muted"}`}>
            <RiBarcodeLine className="text-2xl" />
          </div>
          <div>
            <h3 className="font-black text-text text-sm uppercase tracking-widest">{t.onboardingPage.paymentPending.bankSlip}</h3>
            <p className="text-[10px] text-text-muted font-bold uppercase mt-1">{t.onboardingPage.paymentPending.compensationDesc}</p>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <div className="bg-card rounded-[28px] border border-border shadow-xl overflow-hidden">
             <div className="px-8 py-4 border-b border-border bg-bg-subtle/50 flex items-center gap-2">
                ...
                <h4 className="text-[10px] font-black text-text-muted uppercase tracking-widest">{t.onboardingPage.paymentPending.portalAccessTitle}</h4>
             </div>
             <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                   <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">{t.onboardingPage.paymentPending.login}</p>
                   <div className="flex items-center gap-3 p-4 bg-bg-subtle rounded-2xl border border-border">
                      <RiShieldUserLine className="text-text-muted" />
                      <span className="text-sm font-bold text-text font-mono">{login}</span>
                   </div>
                </div>
                <div className="space-y-2">
                   <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">{t.onboardingPage.paymentPending.password}</p>
                   <div className="flex items-center gap-3 p-4 bg-bg-subtle rounded-2xl border border-border">
                      <RiLockLine className="text-text-muted" />
                      <span className="text-sm font-bold text-text font-mono">{password}</span>
                   </div>
                </div>
             </div>
             <div className="p-4 bg-text text-center">
                <a href="https://ais.usvisa-info.com/pt-br/niv/users/sign_in" target="_blank" rel="noreferrer" className="text-[10px] font-black text-bg uppercase tracking-widest flex items-center justify-center gap-2 hover:underline">
          ...
          <div className="bg-card rounded-[28px] border border-border shadow-xl overflow-hidden">
             <div className="p-8 flex items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h4 className="text-base font-black text-text">{t.onboardingPage.paymentPending.mrvBoletoTitle}</h4>
                      <p className="text-xs text-text-muted font-medium">{t.onboardingPage.paymentPending.mrvBoletoDesc}</p>
                   </div>
                </div>
                {boletoUrl ? (
                   <a
      ...
      {method && (
        <div className="pt-8 border-t border-border">
           ...
           <p className="text-center text-[10px] text-text-muted font-bold uppercase mt-4 tracking-widest">
