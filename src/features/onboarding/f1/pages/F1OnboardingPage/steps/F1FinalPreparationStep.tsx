...
export function F1FinalPreparationStep({ procId, stepData, onComplete }: F1FinalPreparationStepProps) {
  ...
  useEffect(() => {
    async function loadState() {
      ...
      try {
        ...
        console.error("[F1FinalPreparationStep] Error loading state:", err);
      }
    }
  ...
  const handleReportOutcome = async (outcome: 'approved' | 'rejected') => {
    try {
      setLoading(true);
      const nextStepData = await reportInterviewOutcome({ procId, freshStepData, outcome });
      setFreshStepData(nextStepData);
  ...
  const handleOpenSpecialistSupport = async () => {
    ...
    try {
      // Prefer the dedicated mentoria user_service record; fall back to a fresh query
      // when purchasedMentorship wasn't found by the scoped contains() check.
