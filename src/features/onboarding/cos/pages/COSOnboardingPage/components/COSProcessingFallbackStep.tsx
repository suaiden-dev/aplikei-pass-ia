...
export function COSProcessingFallbackStep({
  ...
  const fallbackT = t?.cos?.fallback || {}
  ...
  return (
    <div className='p-16 text-center'>
      ...
      <h2 className='text-xl font-black text-slate-900 mb-2'>
        {fallbackT.title || 'Análise em Andamento'}
      </h2>
      <p className='text-sm text-slate-400 font-medium'>
        {title || fallbackT.adminStep || 'Etapa Administrativa'}:{' '}
        {description || fallbackT.processing || 'Nossa equipe está processando sua solicitação.'}
      </p>
      <div className='mt-8 p-4 bg-slate-50 rounded-2xl border border-slate-100 max-w-sm mx-auto'>
        <p className='text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1'>
          {fallbackT.status || 'Status do Processo'}
        </p>
        <p className='text-xs font-bold text-slate-600'>
          {fallbackT.waitingReview || 'Aguardando revisão interna de nossa equipe.'}
        </p>
      </div>
    </div>
  )
