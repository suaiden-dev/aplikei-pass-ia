...
import type { StripePaymentMethod } from "@features/payments/lib/paymentOps";
import * as paymentService from "@features/payments/lib/paymentOps";
import { useAuth } from "@shared/hooks/useAuth";
import { DocUploadCard } from '@shared/components/molecules/DocUploadCard'
import { ZELLE_RECIPIENT } from "@shared/zelle";
import { Input } from '@shared/components/atoms/input'
import { Label } from '@shared/components/atoms/label'
import { maskCPF, validateCPF } from "@shared/utils/cpf";
import { estimateCardTotal } from "@features/payments/lib/fees";
 ...
 */
export function MotionEndStep({ proc, onMotionResult }: StepProps) {
  const t = useT('onboarding')
  const navigate = useNavigate()
  const { user } = useAuth()
  const data = (proc.step_data as any || {}) as Record<string, unknown>
