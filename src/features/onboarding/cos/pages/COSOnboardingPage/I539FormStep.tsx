...
import { useForm } from "@shared/lib/form/useForm";
import type { FormReturn } from "@shared/lib/form/types";
...
const I539FormContext = createContext<FormReturn<I539FormInput> | null>(null);

function useI539FormContext() {
  const context = useContext(I539FormContext);

  if (!context) {
    throw new Error("I539 form context is unavailable");
  }
...
function flattenValues(
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      flattenValues(item, prefix ? `${prefix}.${index}` : String(index), result);
    });
    return result;
  }
  ...
  if (isPlainObject(value)) {
    Object.entries(value).forEach(([key, nestedValue]) => {
      flattenValues(nestedValue, prefix ? `${prefix}.${key}` : key, result);
    });
    return result;
  }
...
function unflattenValues(flatValues: Record<string, unknown>): Record<string, unknown> {
  ...
  Object.entries(flatValues).forEach(([path, value]) => {
    ...
    keys.forEach((key, index) => {
      ...
      if (isArrayIndex && Array.isArray(current)) {
        ...
        current[currentIndex] ??= nextIsArray ? [] : {};
...
function Field({ label, required, name, children, tooltip }: { label: string; required?: boolean; name?: string; children: ReactNode; tooltip?: string }) {
  ...
  const error = name && form.touched[name] ? (form.errors[name] as string | undefined) : undefined;
...
function SelectInput({ name, options, disabled, children }: {
  ...
  const form = useI539FormContext();
  const field = form.registerSelect(name);
  const error = form.touched[name] ? (form.errors[name] as string | undefined) : undefined;
...
function YesNoGroup({ yesName, noName, disabled }: {
  ...
  const form = useI539FormContext();
  const yesVal = Boolean(form.values[yesName]);
  const noVal = Boolean(form.values[noName]);
      ...
      {(["Yes", "No"] as const).map(opt => {
        ...
        return (
          <button
            ...
            onClick={() => {
              if (opt === "Yes") {
                form.setValue(yesName, true);
                form.setValue(noName, false);
              } else {
                form.setValue(noName, true);
                form.setValue(yesName, false);
              }
            }}
            className={`flex-1 py-3 rounded-xl border text-xs font-black uppercase tracking-widest transition-all ${isSelected
              ? opt === "Yes"
...
type OnboardingTranslations = NonNullable<ReturnType<typeof useT>>;

interface I539DependentViewValue extends Record<string, unknown> {
  id?: string;
  givenName?: string;
  familyName?: string;
}

type I539ViewValues = Record<string, unknown> & {
  hasMiddleName?: boolean;
  hasMailingAddress?: boolean;
  aptSteFlrForeignUnit?: string;
  statusExpiresDS?: boolean;
  priorExtensionYes?: boolean;
  dependentsA?: I539DependentViewValue[];
};

export default function I539FormStep(props: Props) {
  const t = useT("onboarding");

  if (!t || !t.cos) return null;

  return <I539FormStepContent {...props} t={t} />;
}

function I539FormStepContent({ proc, user, onComplete, t }: Props & { t: OnboardingTranslations }) {
  ...
  const form = useForm<I539FormInput>({
    initialValues: flattenValues(initialValues) as I539FormInput,
    validate,
    onSubmit: async (flatValues) => {
      const values = unflattenValues(flatValues) as I539FormInput;
  ...
  const values = useMemo(
    () => unflattenValues(form.values) as I539ViewValues,
    [form.values],
  );
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await processService.updateStepData(proc.id, {
        i539: unflattenValues(form.values),
      });
      ...
      <form id="homologation-form-i539" onSubmit={form.handleSubmit} className="space-y-4 pb-20">
        ...
        {values.dependentsA && values.dependentsA.length > 0 && (
          <div className="pt-10 mt-10 border-t-4 border-slate-100 space-y-8">
            ...
            {values.dependentsA.map((dep: I539DependentViewValue, idx: number) => (
