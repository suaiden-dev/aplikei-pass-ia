          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                <RiChat3Line className="text-4xl text-text-muted/20 mx-auto mb-4" />
                ...
                {threads.map((thread) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'w-full p-4 flex gap-3 text-left transition-all hover:bg-bg-subtle',
        ...
        <div className="hidden md:flex flex-1 bg-bg-subtle/20 flex-col overflow-hidden">
          {selected && !isMobile ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
              <div className="w-24 h-24 rounded-[32px] bg-bg-subtle shadow-lg flex items-center justify-center mb-8">
                <RiChat3Line className="text-5xl text-text-muted/30" />
              </div>
      ...
      <AnimatePresence>
        {selected && selectedProcessId && isMobile && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 z-[100] bg-card flex flex-col"
          >
            <ThreadView
              thread={selected}
              userId={user?.id || ''}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="p-4 md:p-6 bg-card border-b border-border flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              {isClosed ? (
                <>
                  ...
                  <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Encerrado</span>
                </>
              ) : (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block" />
                  <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Ativo</span>
                </>
              )}
            </div>
          </div>
        ...
        <a
          href={`/dashboard/processes/${thread.processRouteSlug || thread.serviceSlug}?slug=${thread.processRouteId || thread.processId}`}
