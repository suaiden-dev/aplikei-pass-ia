...
import { AuthBrand } from "@shared/components/molecules/AuthBrand";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isValid) return;
      ...
      try {
        ...
        setSuccess(true);
      setTimeout(() => navigate("/track-my-visa"), 3000);
    } catch (error) {
      ...
      <AnimatePresence mode="wait">
        {success ? (
          ...
          >
            ...
            <Button className="h-12 w-full rounded-xl font-bold" onClick={() => navigate("/track-my-visa")}>
              {p.goToLogin}
            </Button>
          </motion.div>
        ) : !pendingResetEmail ? (
